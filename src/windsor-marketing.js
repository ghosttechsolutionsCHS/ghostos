...
const SOURCES=Object.freeze({
  googleAds:{connector:'google_ads',preset:'last_30dT',fields:['date','campaign','campaign_id','campaign_status','spend','clicks','impressions','conversions','conversion_value']},
...
export function summarizeGoogleAds(source){
  ...
  const map=new Map();
  for(const row of source.rows){const key=String(row.campaign_id||row.campaign||'unknown');const c=map.get(key)||{id:key,name:String(row.campaign||key),status:String(row.campaign_status||''),spend:0,clicks:0,impressions:0,conversions:0,conversionValue:0};if(row.campaign_status)c.status=String(row.campaign_status);c.spend+=n(row.spend);c.clicks+=n(row.clicks);c.impressions+=n(row.impressions);c.conversions+=n(row.conversions);c.conversionValue+=n(row.conversion_value);map.set(key,c)}
