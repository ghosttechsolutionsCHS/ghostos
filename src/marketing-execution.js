...
const CAMPAIGN_ACTIONS=new Set(['pause_campaign','enable_campaign','set_campaign_budget','set_campaign_bidding_strategy','set_campaign_geo_targeting','set_campaign_language_targeting','set_ad_schedule','set_cpc_bid_ceiling','set_target_cpa','set_target_roas','push_negative_keywords','remove_negative_keywords']);
...
function assertBudget(action,p){
  ...
  if(action==='create_campaign'){
    if(Number(p.budget_amount_micros)>daily)throw new Error('Budget guard blocked configured Google average daily budget above $10/day');
...
export function getManagedGoogleRegistry(rows=[]){
  ...
  for(const r of completedGoogleRows(rows)){
    ...
    if(action==='create_campaign'&&providerId){campaigns.set(providerId,{id:providerId,createdAt:at,configuredAverageDailyBudgetUsd:Number(payload.budget_amount_micros||0)/1_000_000,sourceRecordId:r.id,firstEnabledAt:null,lastControlAction:'create_campaign',lastControlAt:at});}
  ...
  for(const r of ordered){
    const f=r.fields||{},action=String(f['Execution Action']||''),payload=parsePayload(f['Execution Payload JSON']),campaignId=String(payload.campaign_id||''),at=f['Execution Completed At']||f['Execution Started At']||f['Created At']||r.createdTime||null,c=campaigns.get(campaignId);
    if(!c)continue;
...
export function assertManagedGoogleTarget(action,payload={},registry){
  if(action==='create_campaign')return true;
  if(!registry)throw new Error('GhostOS managed Google Ads registry is required');
  ...
  if(CAMPAIGN_ACTIONS.has(action)){
    const id=String(payload.campaign_id||'');if(!registry.campaignIds.has(id))throw new Error('Blocked Google Ads action targeting a campaign not registered as GhostOS-managed');return true;
  }
...
function managedSpendSnapshot(source,registry,now=new Date()){
  ...
  const managedRows=(source.rows||[]).filter(r=>registry.campaignIds.has(String(r.campaign_id||'')));
  const statusByCampaign=new Map();
  for(const row of managedRows){if(row.campaign_status)statusByCampaign.set(String(row.campaign_id),String(row.campaign_status).toUpperCase())}
  const activeCampaignIds=[...registry.campaigns.values()].filter(c=>statusByCampaign.get(c.id)==='ENABLED'||(!statusByCampaign.has(c.id)&&c.lastControlAction==='enable_campaign')).map(c=>c.id);
  const launchTimes=[...registry.campaigns.values()].map(c=>Date.parse(c.firstEnabledAt||'')).filter(Number.isFinite);const launchAt=launchTimes.length?Math.min(...launchTimes):null,firstSevenEnd=launchAt==null?null:launchAt+7*86400000;
  const today=localDate(now);let todayObservedSpend=0,firstSevenDayObservedSpend=0;
...
async function surfaceSpendAlert({rows,summary,amount,detail}){
  const approvals=await listRecords(TABLES.APPROVALS,{maxRecords:200});
  const duplicate=approvals.some(r=>r.fields?.Status==='Pending'&&String(r.fields?.Summary||'')===summary);if(duplicate)return false;
  await createApproval({type:'Marketing Spend Anomaly',amount,summary,requestedAction:detail,requestedBy:'FORGE'});
  await logActivity({agent:'ATLAS',actionType:'google_ads_spend_owner_attention',status:'Blocked',detail:`${summary}. ${detail}`,consequential:true});return true;
}
...
async function executeWindsor({connector,action,payload,env=process.env,fetchImpl=fetch,registry=null}){
  ...
  validateExecutionSpec({connector,action,payload});if(connector==='google_ads')assertManagedGoogleTarget(action,payload,registry);
...
export async function monitorManagedGoogleSpend({rows,now=new Date(),env=process.env,fetchImpl=fetch}={}){
  ...
  const source=await createWindsorMarketingReader({env,fetchImpl}).readSource('googleAds');const observed=managedSpendSnapshot(source,registry,now);
  const managed=[...registry.campaigns.values()];
  const budgetViolation=managed.find(c=>c.configuredAverageDailyBudgetUsd>MARKETING_AUTH.configuredAverageDailyBudgetMaxUsd);
  if(budgetViolation)await surfaceSpendAlert({rows,summary:`GhostOS-managed Google campaign ${budgetViolation.id} has a configured average daily budget above $10`,amount:budgetViolation.configuredAverageDailyBudgetUsd,detail:'Owner attention required. GhostOS will not increase this budget; review the provider configuration. Google average daily budget is not a guarantee of a calendar-day charge cap.'});
  if(observed.activeCampaignIds.length&&!observed.reportingAvailable){await surfaceSpendAlert({rows,summary:'Google Ads spend reporting unavailable for an active GhostOS-managed campaign',amount:null,detail:'FORGE cannot reliably observe spend right now. Provider reporting can lag; the $70 protection is best-effort and cannot guarantee against delayed platform-reported spend.'});return {...observed,registry,alerted:true};}
  if(observed.todayObservedSpend>MARKETING_AUTH.configuredAverageDailyBudgetMaxUsd){await surfaceSpendAlert({rows,summary:`Observed Google Ads spend is $${observed.todayObservedSpend.toFixed(2)} today`,amount:observed.todayObservedSpend,detail:'Observed calendar-day spend exceeded the configured $10 average daily budget. Google may overdeliver relative to an average daily budget; this is an anomaly alert, not proof the configured budget changed.'});}
  const thresholdReached=observed.firstSevenDayObservedSpend>=MARKETING_AUTH.firstSevenDaysObservedSpendMaxUsd;
  const paused=[];if(thresholdReached){
    for(const campaignId of observed.activeCampaignIds){try{await executeWindsor({connector:'google_ads',action:'pause_campaign',payload:{campaign_id:campaignId},env,fetchImpl,registry});paused.push(campaignId);}catch(error){await surfaceSpendAlert({rows,summary:`Failed to pause GhostOS-managed Google campaign ${campaignId} after observed first-7-day spend reached $${observed.firstSevenDayObservedSpend.toFixed(2)}`,amount:observed.firstSevenDayObservedSpend,detail:`Immediate owner attention required. ${safeError(error,env.WINDSOR_API_KEY)}`});}}
    await surfaceSpendAlert({rows,summary:`Observed first-7-day Google Ads spend reached $${observed.firstSevenDayObservedSpend.toFixed(2)}`,amount:observed.firstSevenDayObservedSpend,detail:`GhostOS paused ${paused.length} active GhostOS-managed campaign(s). This $70 protection is a best-effort execution guard because Google/Windsor reporting can lag; it is not a guarantee against spend reported later.`});
  }
...
export async function runMarketingExecutionCycle({now=new Date(),env=process.env,fetchImpl=fetch}={}){
  ...
  const rows=await listRecords(TABLES.GROWTH_WORK,{maxRecords:300});const registry=getManagedGoogleRegistry(rows);
  const spendMonitor=await monitorManagedGoogleSpend({rows,now,env,fetchImpl});
  ...
  const row=due[0];if(!row)return {skipped:true,reason:'no_due_approved_execution',spendMonitor};
  ...
  if(connector==='google_ads'&&action==='enable_campaign'){
    ...
    if(spendMonitor.reportingAvailable===false&&registry.campaigns.get(String(payload.campaign_id||''))?.firstEnabledAt)throw new Error('Spend guard blocked re-enablement because current observed spend reporting is unavailable');
  }
  ...
  try{
    const result=await executeWindsor({connector,action,payload,env,fetchImpl,registry});const meta=providerMeta(result),done=new Date().toISOString();
    ...
    await logActivity({agent:connector==='google_ads'?'FORGE':'ECHO',actionType:'marketing_execution_completed',status:'Done',detail:`Provider confirmed ${connector}/${action} for ${f['Growth Item']||row.id}.`,consequential:true});return {skipped:false,id:row.id,completed:true,connector,action,providerId:meta.id,spendMonitor};
