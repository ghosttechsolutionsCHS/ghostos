...
export default async (req) => {
  ...
  try {
    ...
    if (body.action === 'reject') return json({ ok: true, request: await rejectProposal(body.requestId) });
    if (body.action === 'approve_merge') return json({ ok: true, request: await approveAndMergeProposal(body.requestId) });
